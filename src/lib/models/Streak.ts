...
export class Streak {
	...
	public static async delete(
		...
		for (let i = 0; i < ids.length; i++) {
			const streak = db.streaks.findIndex(
				(streak) => streak.memberid === ids[i]
			);
			if (streak === -1) {
				...
				if (!sorted[parseInt(ids[i]) - 1]) {
					...
					const member = await message.client.fetchUser(
						sorted[parseInt(ids[i]) - 1].memberid
					);
					return {
						code: "IDENTIFIED",
						message: `user identified by rank ${
							ids[i]
						}. \nType \`!admin delete ${
							sorted[parseInt(ids[i]) - 1].memberid
	...
	public static async old(
		...
		const streaks = db.streaks
			.map((streak) =>
				Object.assign({}, streak, {
					age:
						new Date().getTime() -
						streak.streak -
						streak.getStartDate().getTime(),
					name: (
						message.guild.members.get(streak.memberid) || {
			...
			.slice(start, end);
		const maxLength = streaks.reduce(
			(carry, streak) => Math.max(carry, streak.name.length),
			0
		);
		return (
			"```" +
