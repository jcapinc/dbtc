...
export class Database {
	...
	public static parse(value: any): Database {
		...
		db.streaks = value.streaks
			? value.streaks.map((s) => Object.assign(new Streak(), s))
			: [];
		db.profile = value.profile
			? value.profile.map((s) => Object.assign(new StreakGroup(""), s))
			: [];
		db.settings = value.settings
			? Object.assign({}, db.settings || {}, value.settings)
			: {};
		return db;
	}
