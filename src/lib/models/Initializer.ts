...
export default class Initializer {
	...
	public async initialize(profile?: Array<StreakGroup>): Promise<Initializer> {
		const guildMember = await this.message.guild.fetchMember(
			this.message.author
		);
		...
		await Promise.all(
			profile.map<Promise<void>>((g) => this.initializeGroup(g))
		);
	...
	private async generateStreakGroup(): Promise<Array<StreakGroup>> {
		this.categoryChannel = await this.message.guild.createChannel("Streaks", {
			type: "category",
		});
...
export class StreakGroup {
	...
	async initialize(guild: Guild): Promise<void> {
		...
		this.generatedChannel = await this.initializeChannel(
			guild,
			this.generatedRole
		);
	}
	...
	static async list() {
		...
		const maxLength = profiles.reduce(
			(carry, record) => Math.max(carry, record.name.length + 1),
			0
		);
		const columns = ["Streak Group", "End Days"];
		const title =
			`##  ${columns[0]}${" ".repeat(maxLength - columns[0].length)}${
				columns[1]
			}\n` +
			`--  ${"-".repeat(columns[0].length)}${" ".repeat(
				maxLength - columns[0].length
			)}${"-".repeat(columns[1].length)}\n`;
		return profiles.reduce(
			(carry, record, index) =>
				carry +
				" ".repeat(2 - index.toString().length) +
				index.toString() +
				`  ${record.name}${" ".repeat(maxLength - record.name.length)}${
					record.days
				}\n`,
			title
		);
